...
cols=['subject','side','chan','depth','zeroCross','peakPos','wamp']
...
subs_done = np.unique([x.split('_')[0].strip('sub-') for x in os.listdir(out_path) if os.path.isfile(os.path.join(out_path,x))]).tolist()

subs_ignore=['']

for isubject in sorted_nicely([x for x in layout.get_subjects()[::-1][9:] if x not in subs_done+subs_ignore])[::-1]:
	...
	if sub_data['target'].values[0].lower() =='stn':
		...
		for iedf in edf_files:
			f = pyedflib.EdfReader(iedf)
			equipment = f.equipment.decode("utf-8") 
			annots=f.readAnnotations()
			...
			if iside is not None:
				for ichan in range(num_chans):
					...
					if not os.path.exists(os.path.join(out_path,fileName+".png")):
						...
						print(f'{isubject} {sigbufs[ichan,:].max()} {sigbufs[ichan,:].min()}')
						nDepths = len(depths)
						yshift=round((max(abs(sigbufs[ichan,:].max()),abs(sigbufs[ichan,:].min())))/100,2)
						
						if sigbufs[ichan,:].max() > 50:
							yshift= 100
						else:
							yshift= .5
...
for isubject in subjects:
	...
	sub_data=surgical_data[surgical_data['subjectNumber']==isub]
	if sub_data['target'].values[0].lower() =='stn':
		sub_plot_out=os.path.join(out_plot_path,isubject)
